...
@interface XLsn0wAddressPicker () <UIPickerViewDataSource, UIPickerViewDelegate>
...
@implementation XLsn0wAddressPicker
...
- (void)selectedOk {
    if (self.pickerBlock) {
        self.pickerBlock(self, self.province, self.city, self.area);
    }
    [self remove];
}
...
{
    if (!_arrayRoot) {
        NSString *plist_path = [[NSBundle mainBundle] pathForResource:@"XLsn0wAddressPicker" ofType:@"plist"];
        _arrayRoot = [[NSArray alloc] initWithContentsOfFile:plist_path];
    }
    return _arrayRoot;
}
